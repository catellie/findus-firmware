...
Do not create the enrollment bundle yet. It is deliberately short-lived and
should be created only after the Findus firmware is installed and ready.
...
1. Restart the newly installed device and wait until the `Findus-XXXX` Wi-Fi
   network is visible. Stay on the normal internet connection for now.
2. Return to **Findus > Set up Findus**, enter the child or owner's name in
   stage 5, create the 30-minute enrollment bundle, and copy it.
3. Join `Findus-XXXX` from the phone or computer.
4. Select the household Wi-Fi network and enter its password.
5. Paste the complete enrollment bundle copied from the Sheet.
6. Submit the form and reconnect the phone or computer to its normal Wi-Fi.
